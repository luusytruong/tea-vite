    ...
    >
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          ...
          >
            <div className={`absolute inset-0 bg-gradient-to-b ${currentTheme.bg}`} />
          </motion.div>
          ...
          <div className="relative h-full container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="h-full grid grid-cols-1 lg:grid-cols-2 items-center gap-8 lg:gap-12 pt-16 pb-20 lg:py-0">
              ...
              >
                <span className={`inline-block text-base lg:text-lg ${currentTheme.accent} font-medium tracking-wide`}>
                  {slides[currentSlide].subtitle}
                </span>
                
                <h1 className={`text-5xl md:text-5xl lg:text-6xl font-medium old-style ${currentTheme.text} leading-tight`}>
                  {slides[currentSlide].title}
                </h1>
                
                <p className={`text-base md:text-lg ${currentTheme.text} opacity-90 leading-relaxed`}>
                ...
                {slides[currentSlide].stats && (
                  <div className="grid grid-cols-3 gap-3 md:gap-4 py-2 md:py-4">
                    {slides[currentSlide].stats.map((stat, index) => (
                      ...
                      >
                        <div className={`text-xl md:text-2xl lg:text-3xl font-medium ${currentTheme.accent}`}>
                          {stat.value}
                        </div>
                ...
                {slides[currentSlide].collection && (
                  <motion.div
                    ...
                    className="space-y-3"
                  >
                    <h3 className={`text-lg font-medium ${currentTheme.accent}`}>
                      {slides[currentSlide].collection.title}
                    </h3>
                    <div className="flex flex-wrap gap-2">
                      {slides[currentSlide].collection.items.map((item, index) => (
                        <span
                          key={index}
                ...
                >
                  ...
                  <Link
                    to={ROUTES.ABOUT}
                    className={`px-6 py-2.5 rounded-xl border text-sm md:text-base transition-colors duration-300 font-medium ${currentTheme.buttonOutline} hover:scale-105`}
                  >
              ...
              >
                <div className="absolute inset-0 rounded-xl md:rounded-2xl overflow-hidden shadow-2xl">
                  <motion.img
